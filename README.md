...
## Seguridad (DevSecOps básico)

- Puertos publicados solo en `127.0.0.1` (no accesibles desde la red)
- Red interna dedicada (`ia-net`) para la comunicación entre servicios
- Contenedores con `no-new-privileges` y `cap_drop: ALL`
- Imágenes con versión fijada (Ollama `0.5.7`, Open WebUI `v0.5.20-cuda`)
- Registro sin altas abiertas (`ENABLE_SIGNUP=false`)
- Pipeline CI (`.github/workflows/ia-server-security.yml`): escaneo de secretos con Gitleaks, validación de compose, lint del Dockerfile (Hadolint) y escaneo de vulnerabilidades (Trivy)
